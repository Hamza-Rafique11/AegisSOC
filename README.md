...
Edit .env and set GEMINI_API_KEY if you want live LLM-powered incident investigations (see Environment Variables). The platform works fully without it, using the offline deterministic analysis engine as a fallback.
...
RULE_ONLY — signature/rule-based detection alone
ML_ONLY — unsupervised anomaly detection alone
RULE_PLUS_ML — combined rule + ML pipeline
RULE_ML_AI_ANALYST — full pipeline with AI-assisted triage and investigation
